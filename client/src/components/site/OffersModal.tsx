...
import { useLocalizedOffer } from "@/i18n/contentHooks";
...
function OfferCard({
  ...
  const localized = useLocalizedOffer(offer);
  ...
  return (
    ...
    >
      ...
      <div className="mb-4 flex items-start justify-between gap-3">
        <p className="eyebrow text-[var(--gold)]">{localized.period}</p>
      ...
      <MagneticButton className="premium-btn w-full" onClick={onBook}>
        {t("offers.claimCta", "Възползвай се")}
      </MagneticButton>
    </motion.article>
  );
}
          ...
          <div className="relative mt-8 grid gap-6 md:grid-cols-2">
            {isLoading && (
              <p className="col-span-full text-center text-white/60">
                {t("offers.loading", "Зареждане...")}
              </p>
            )}
            {!isLoading && offers.length === 0 && (
              <p className="col-span-full text-center text-white/60">
                {t("offers.empty", "В момента няма активни оферти.")}
              </p>
            )}
            {offers.map((offer, idx) => (
              <OfferCard key={offer.id} offer={offer} idx={idx} onBook={handleBooking} />
            ))}
          </div>
