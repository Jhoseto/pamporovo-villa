...
export function AdminPwaInstallBanner({ placement = "app" }: { placement?: "app" | "login" }) {
  const { canInstall, isInstalling, install, showBanner, dismissBanner, showIosInstructions, isIos } =
    useAdminPwaInstall();
  ...
  return (
    <div className={`admin-pwa-banner fixed inset-x-0 ${positionClass} z-40 mx-3 lg:hidden`}>
      <div className="admin-glass-card flex items-start gap-3 p-4 shadow-lg">
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[var(--admin-glass-strong)]">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-xs text-[var(--admin-muted)]">
            {showIosInstructions
              ? "Добави на началния екран за пълен екран без браузър."
              : "Отвори админ панела като самостоятелно приложение с известия."}
          </p>
          <div className="mt-3 flex flex-wrap gap-2">
            {canInstall ? (
              <Button size="sm" className="admin-btn-primary h-8" onClick={() => install()} disabled={isInstalling}>
                <Download className="mr-1.5 h-3.5 w-3.5" />
                {isInstalling ? "Инсталиране..." : "Инсталирай"}
              </Button>
            ) : showIosInstructions ? (
              ...
              <span className="text-xs text-[var(--admin-muted)]">Chrome → Меню → Инсталирай приложението</span>
            )}
